-- ============================================================
-- Fix: add missing INSERT policies (idempotent)
-- ============================================================

-- HOUSEHOLDS
DROP POLICY IF EXISTS "Authenticated users can create households" ON households;
CREATE POLICY "Authenticated users can create households"
  ON households FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

-- TRANSACTION CATEGORIES
DROP POLICY IF EXISTS "Members can insert categories" ON transaction_categories;
CREATE POLICY "Members can insert categories"
  ON transaction_categories FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- SHOPPING LISTS
DROP POLICY IF EXISTS "Members can insert shopping lists" ON shopping_lists;
CREATE POLICY "Members can insert shopping lists"
  ON shopping_lists FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- SHOPPING ITEMS
DROP POLICY IF EXISTS "Members can insert shopping items" ON shopping_items;
CREATE POLICY "Members can insert shopping items"
  ON shopping_items FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- RECIPES
DROP POLICY IF EXISTS "Members can insert recipes" ON recipes;
CREATE POLICY "Members can insert recipes"
  ON recipes FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- MEAL PLANS
DROP POLICY IF EXISTS "Members can insert meal plans" ON meal_plans;
CREATE POLICY "Members can insert meal plans"
  ON meal_plans FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- MEAL PLAN ENTRIES
DROP POLICY IF EXISTS "Members can insert meal plan entries" ON meal_plan_entries;
CREATE POLICY "Members can insert meal plan entries"
  ON meal_plan_entries FOR INSERT
  WITH CHECK (
    plan_id IN (SELECT id FROM meal_plans WHERE household_id = get_household_id())
  );

-- SAVINGS GOALS
DROP POLICY IF EXISTS "Members can insert savings goals" ON savings_goals;
CREATE POLICY "Members can insert savings goals"
  ON savings_goals FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- TRANSACTIONS
DROP POLICY IF EXISTS "Members can insert transactions" ON transactions;
CREATE POLICY "Members can insert transactions"
  ON transactions FOR INSERT
  WITH CHECK (household_id = get_household_id() AND user_id = auth.uid());

-- WORKOUT PLANS
DROP POLICY IF EXISTS "Users can insert workout plans" ON workout_plans;
CREATE POLICY "Users can insert workout plans"
  ON workout_plans FOR INSERT
  WITH CHECK (user_id = auth.uid() AND household_id = get_household_id());

-- EXERCISES
DROP POLICY IF EXISTS "Users can insert exercises" ON exercises;
CREATE POLICY "Users can insert exercises"
  ON exercises FOR INSERT
  WITH CHECK (
    plan_id IN (SELECT id FROM workout_plans WHERE user_id = auth.uid())
  );

-- WORKOUT LOGS
DROP POLICY IF EXISTS "Users can insert workout logs" ON workout_logs;
CREATE POLICY "Users can insert workout logs"
  ON workout_logs FOR INSERT
  WITH CHECK (user_id = auth.uid() AND household_id = get_household_id());

-- WORKOUT LOG EXERCISES
DROP POLICY IF EXISTS "Users can insert workout log exercises" ON workout_log_exercises;
CREATE POLICY "Users can insert workout log exercises"
  ON workout_log_exercises FOR INSERT
  WITH CHECK (
    log_id IN (SELECT id FROM workout_logs WHERE user_id = auth.uid())
  );

-- CHILDREN
DROP POLICY IF EXISTS "Members can insert children" ON children;
CREATE POLICY "Members can insert children"
  ON children FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- CHILD EVENTS
DROP POLICY IF EXISTS "Members can insert child events" ON child_events;
CREATE POLICY "Members can insert child events"
  ON child_events FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- VACCINES
DROP POLICY IF EXISTS "Members can insert vaccines" ON vaccines;
CREATE POLICY "Members can insert vaccines"
  ON vaccines FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- PETS
DROP POLICY IF EXISTS "Members can insert pets" ON pets;
CREATE POLICY "Members can insert pets"
  ON pets FOR INSERT
  WITH CHECK (household_id = get_household_id());

-- PET EVENTS
DROP POLICY IF EXISTS "Members can insert pet events" ON pet_events;
CREATE POLICY "Members can insert pet events"
  ON pet_events FOR INSERT
  WITH CHECK (household_id = get_household_id());
